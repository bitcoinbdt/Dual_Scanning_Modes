/**
 * Birdeye API Client - Fetches OHLCV (candlestick) data for BSC
 * Similar to Solana version but with 'bsc' chain
 */
...
/**
 * Fetch OHLCV (candlestick) data from Birdeye API for BSC
 * @param address - BSC token address
...
export async function fetchOHLCV(
  ...
  const response = await axios.get(url, {
    headers: {
      'X-API-KEY': apiKey,
      'x-chain': 'bsc' // BSC chain instead of Solana
    },
    params: {
      address: address,
      type: '15m',
