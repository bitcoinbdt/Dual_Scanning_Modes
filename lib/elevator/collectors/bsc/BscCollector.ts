  ...
  async collect(address: string, maxTransactions: number, tokenDecimals?: number): Promise<CollectorResult> {
    ...
    try {
      ...
      try {
        ...
        console.warn(`[BscCollector] OHLCV fetch failed: ${err.message}`);
      }
      ...
      try {
        ...
        console.warn(`[BscCollector] Swap transactions fetch failed: ${err.message}`);
      }

      if (transactions.length === 0) {
        console.warn('[BscCollector] Warning: No swap transactions found for this token on BSC');
      }
