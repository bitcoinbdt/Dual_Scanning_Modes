/**
 * Birdeye API Client - Fetches OHLCV (candlestick) data for Ethereum
 * Similar to BSC version but with 'ethereum' chain
 */
...
/**
 * Fetch OHLCV (candlestick) data from Birdeye API for Ethereum
 * @param address - Ethereum token address
...
export async function fetchOHLCV(
  ...
  const response = await axios.get(url, {
    headers: {
      'X-API-KEY': apiKey,
      'x-chain': 'ethereum' // Ethereum chain
    },
    params: {
      address: address,
      type: '15m',
