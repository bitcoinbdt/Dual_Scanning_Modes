...
/**
 * Calculate FIFO (First-In-First-Out) cost basis P&L for a specific wallet
 */
export function calculateFIFOPnL(
  ...
  for (const sell of sellLots) {
    ...
    while (sellAmount > 0 && activeLots.length > 0) {
      ...
      const oldestLotTotalFees = oldestLot.gasCostUsd + oldestLot.dexFeeUsd;

      if (oldestLot.amount <= sellAmount) {
        realizedPnL += oldestLot.amount * (sellPrice - oldestLot.price);
        realizedFees += (oldestLot.amount / oldestLot.origAmount) * oldestLotTotalFees;
        sellAmount -= oldestLot.amount;
        activeLots.shift();
      } else {
        realizedPnL += sellAmount * (sellPrice - oldestLot.price);
        realizedFees += (sellAmount / oldestLot.origAmount) * oldestLotTotalFees;
        oldestLot.amount -= sellAmount;
        sellAmount = 0;
      }
    }
