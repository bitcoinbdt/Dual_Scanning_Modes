...
export function Referrals() {
    ...
    supabase
      ...
      .then(({ data }) => {
        if (data) {
          ...
          setTotalEarned(data.reduce((s, r) => s + r.bonus_credits, 0));
        }
      });
  }, [user, navigate]);
  ...
  return (
    <div className="min-h-screen bg-grid">
        ...
        <div className="glass-strong rounded-2xl p-6 mb-6 rgb-border">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div>
              ...
              <label className="text-xs text-muted-themed mb-2 mt-4 block">Referral Link</label>
              <div className="flex items-center gap-2">
                <input
                  readOnly
                  value={referralLink}
                  className="flex-1 bg-themed border border-themed rounded-lg px-3 py-2.5 text-xs text-themed font-mono focus:outline-none"
                />
                <button
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="glass rounded-xl p-4 text-center">
                ...
                <div className="text-3xl font-bold gradient-text">{totalReferrals}</div>
              ...
              <div className="glass rounded-xl p-4 text-center">
                ...
                <div className="text-3xl font-bold gradient-text">{totalEarned}</div>
              ...
              <tbody>
                {REFERRAL_TIERS.map((tier) => (
                  <tr key={tier.name} className="border-b border-white/5 hover:bg-white/5">
                    <td className="py-3 px-3 font-semibold text-themed relative">
                      {tier.name}
                      {tier.hot && (
            ...
            {REFERRAL_TIERS.map((tier) => (
              <div key={tier.name} className="glass rounded-lg p-3">
                ...
                <div className="grid grid-cols-3 gap-2 text-center text-xs">
                  ...
                  <div>
                    <div className="text-muted-themed">Percent</div>
      ...
      <ReferralHistoryModal open={showHistory} onClose={() => setShowHistory(false)} />
    </div>
  );
}
